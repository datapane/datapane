...
class Interactive(BaseElement):
    ...
    def __init__(
        ...
        # prob ok for now
        self.function_id = f"app.{id(function)}"
...
class Dynamic(BaseElement):
    """
    A Lazily-evaluated block which is evaluated on each app render
    (essentially a shortcut for interactive with an onload and no parameter)
    NOTE - currently unused
    """

    _tag = "Dynamic"

    # TODO - do we put other params here????
    def __init__(self, function: t.Callable[..., View]):
        self.function = function
        super().__init__()
